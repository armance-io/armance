...
async def test_import_doc_auto_index_true(client: AsyncClient, armance_root: Path) -> None:
    ...
    with patch("armance.web.backend.routes.library_docs.sync_docs") as mock_sync:
...
async def test_import_doc_too_large_returns_413(client: AsyncClient) -> None:
    ...
    from armance.web.backend.routes.library_docs import MAX_UPLOAD_BYTES
...
async def test_import_doc_uses_storage_write_bytes(
    ...
    with patch("armance.web.backend.routes.library_docs.LocalFilesystemStorage", return_value=FakeStorage()):
        with patch("armance.web.backend.routes.library_docs.sync_docs", return_value={}):
            resp = await client.post(
                f"/projects/default/sessions/{sid}/library/docs",
                data={"auto_index": "false"},
