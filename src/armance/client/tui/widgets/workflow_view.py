"""DAG ASCII renderer and workflow TUI helpers.

- render_dag_ascii: produces a compact top-down ASCII DAG from step dicts.
- format_claim_density: header string for claim counts.
- classify_agent_kind: maps agent label to role category (synthesis/critique/specialist).

Spec: docs/spec/21_workflow_design.md § DAG ASCII rendering
      docs/spec/19_claim_ledger.md § TUI surfacing
"""
...
# ---------------------------------------------------------------------------
# Claim density header
# ---------------------------------------------------------------------------

def format_claim_density(
    total: int,
    verified: int,
    disputed: int,
    unsourced: int,
) -> str:
    """Return a one-line summary like '4 claims · 2 ✓ verified · 1 ! disputed · 1 unsourced'."""
    return (
        f"{total} claims · {verified} ✓ verified · {disputed} ! disputed · {unsourced} unsourced"
    )


# ---------------------------------------------------------------------------
# Agent kind classifier for TUI colour coding
# ---------------------------------------------------------------------------
...
def classify_agent_kind(label: str) -> str:
    """Return 'synthesis', 'critique', or 'specialist' from agent label string."""
    if _MONA_PATTERNS.search(label):
        return "synthesis"
    if _SERGE_PATTERNS.search(label):
        return "critique"
    return "specialist"
