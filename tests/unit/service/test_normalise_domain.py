...
from armance.service.agents.recruiter_agent import _normalise_domain


def test_keeps_clean_english_slug() -> None:
    assert _normalise_domain("historian") == "historian"


def test_strips_french_stopwords_and_accents() -> None:
    out = _normalise_domain("Historien des temps modernes")
...
def test_event_organizer_french_normalised() -> None:
    out = _normalise_domain("coordinateur événementiel")
    assert out == "coordinateur-evenementiel"


def test_multi_word_kebab_case() -> None:
    assert _normalise_domain("Project Manager") == "project-manager"
...
def test_punctuation_collapses_to_hyphen() -> None:
    out = _normalise_domain("coordinateur (logistique)")
    assert "(" not in out
    assert ")" not in out
...
def test_long_phrase_truncated_to_three_tokens() -> None:
    out = _normalise_domain("expert en histoire culturelle des relations franco-ecossaises")
